  ...
  public DistanceResponse calculateDistance(DistanceRequest request) {
    ...
    for (int i = 0; i < placeIds.size() - 1; i++) {
      ...
      Place place1 = placeRepository.findById(placeIds.get(index))
        .orElseThrow(
          () -> new IllegalArgumentException("Place not found with id: " + placeIds.get(index)));
      Place place2 = placeRepository.findById(placeIds.get(index + 1))
        .orElseThrow(
          () -> new IllegalArgumentException(
            "Place not found with id: " + placeIds.get(index + 1)));

      double distance = haversine(place1.getLat(), place1.getLng(), place2.getLat(),
        place2.getLng());
